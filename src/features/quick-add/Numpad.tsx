...
export function Numpad({ onDigit, onDelete, onConfirm, onShortcut, canConfirm, isSaving, isNote }: NumpadProps) {
  ...
  const accent = isNote ? '#6366F1' : (settings.accentColor || '#E8A020')
      ...
      <div className="flex gap-1.5 overflow-x-auto scrollbar-hide pb-0.5">
        {SHORTCUTS.map((s) => (
          ...
          onClick={() => { triggerHaptic('light'); onShortcut?.(s.amount); }}
            className="shrink-0 h-8 px-3 rounded-xl bg-white shadow-sm text-[12px] font-bold text-text-muted active:bg-surface border border-border/30"
          >
            {s.label}
          </motion.button>
        ))}
